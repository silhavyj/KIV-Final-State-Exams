...
- presuny dat (pristup do pameti)
  ...
  - word size = mnozstvi bitu ktere CPU dokaze zpracova najednou (v dnesni dobe typicky 64 = 8B)
    - data aligment urcuje jak jsou data zarovnana v pameti => typicky chceme nasobky strojoveho slova => nemusime data nacitat na vicekrat
...
- OpenCL
  ...
  - priklad: soucet vektoru na CPU vs GPU (OpenCL)
    ...
    - work-group obsahuje work itemy
      ...
      - global work group size
        - celkova veliksot work group (itemy v teto workgroupe se nemusi provest na stejne vypocetni jednotce)
  ...
  - OpenCL pouziva buffery pro operace nad daty
    - buffer je potreba vytvorit pres `clCreateBuffer()`; zle take specifikovat jeho chovani (parametry)
...
- redukcni operace
  ...
  - OpenCL memory
    ...
    - privatni pamet
      ...
      - nadmerny objem privatne deklarovane pameti bude namapovan na lokalni (a nasledne na globalni) pamet
  ...
  - OpenCL barrier a fence
    - fence
      - zarucuje ze vsechny instrukce prcitani/ukladani (read-write isntrukce) budou dokonceny pred fence
    ...
    - work groupy lze synchronizovat pouze rozdelenim jader
      ...
      - pretrvava pouze blobalni pamet
...
- faktory pro urychleni
  ...
  - synchronizace muze byt draha
    - zase jsou problem wavefronty -> synchronizace pro work item provadime uvnitr work groupy ale ta muze byt rozdelena na nekolik wavefront
    - problem s wavefronty lze rezit ze `|work group| == |wave front|`
    - lze to vycist z `device.get_info()`
  - dalsim faktorem je mnozstvi dat
    - pro bloky muze byt rezie spojena s kernelem a OpenCL velka v porovnani se vyslednym urcyhlenim
  ...
  - overprovisioning
    - = vytvorzeni vice work-groups nez je kapacita GPU
...
- C++ AMP (= C++ Accelerated Massive Parallelism)
  ...
  - C++ AMP klade omezeni co se tyce toho co funkce (kod) oznaceny klicovym slovem `restrict` muze obsahovat
