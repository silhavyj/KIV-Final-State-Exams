...
- VFS (= Virtual File System)
  ...
  - obsahuje ukazatele na funkce (obdoba C rozhrani)
    - v podstate se v terminologii OOP jedna o dedicnost (superclass CFile_System od ktere dedime a prpisujeme metody tupu `open`, `read`, `write` a `close`)
...
- IFS (= Installable File System)
  ...
  - pro komunikaci se pouziva IORP (= I/O Request Packet)
    ...
    5) az zarizeni odpovi, posle vysledek danemu procesu v userspace (napr pres EAX - navratova hodntoa)
...
- obsluha IRQ
  ...
  - udalost oznamujici OS ze se ma dany HW obslouzit (neco se stalo = udalost)
    - zmacknuta klavesnice, prisel packet, tick casovace, deleni 0, atd.
  ...
  - sdileni jednoho IRQ (konflikty)
    ...
    - systematicke reseni => MSI (= Message-Signaled Interrupts)
      ...
      - => muzeme rozlisit vicero zarizeni pomoci jednoho IRQ -> Advanced PIC (= APIC)
        - umi preposila mnohem vice IRQ nez obycejny PIC
  ...
  - pro rychlost a responzivnost se IRQ deli na top-half a bottom-half
    ...
    - alternativneL Linux ma WorkQueue - dalsi zpusob odkladani prace ktera neni urgentni
  - ne vzdy musime vyuzivat presuseni
    ...
    - polling
      - typicky aktivni cekani (blokace) na zmenu stavoveho bitu
...
- FAT (= File Allocation Table)
  ...
  - directory obsahuje: nazev soiboru, index 1. clusteru, + metadata (napr jestli se jedna o soubor nebo slozku)
...
- Ext2
  ...
  - ext2 se zklada z nekolika velkych bloku - boot bloku a pak N block group => kazda ma vlastni superblock, bitmapy a clustery (datove bloky)
...
- NTFS
  ...
  - podporuje zurnalovani (= zaznam akci co se chystam udelat nez je udelam), ACL, kompresi, sifrovani, kvoty (= omezeni obsazeneho mista ve FS ktery nastavi administrator), hard a symlinky
