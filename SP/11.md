...
- ILP (= Instruction Level Parallelism)
  ...
  - dva pristupy
    - zavisi na HW a vyuziva paralelismus dynamicky
  ...
  - paralelni muze provaded instrukce, ktere jsou na sobe nezavisle
  - existuje vicero technik a principu ILP
    ...
    - out-of-order execution (vykonani instrukci v prehazenem poradi)
      - mysi byt datove nezavisle (napr. necekame na vysledek predchozi isntrukce)
    - predikce skoku
...
- pipelining
  ...
  - je mozne provaded vicero isntrukci najednou resp. vice instrukci v ruznych stavech zpracovani
  ...
  - hazardy
    ...
    - datove hazardy (= pametove zavislosti) - instrukce zavisi na vysledku predchozi instrukce ktera je stale v pipeline
      ...
      - WAW (= Write After Write) vyresime pouzitim jineho registru
        ...
        - reseni na urovni prekladace nebo HW (napr Scoreboarding - dynamicke resni za runtime)
    - ridici hazardy - zpusobene zpozdenim mezi nactenim instrukce a rozhodnutim o zmene instrukcniho toku (vetveni & skoky)
  - procesory jsou schopni provaded out-of-order execution instrukci
...
- Predikce vetveni/skoku
  - diky pipeline muzeme zpracovavat vicero instrukci najednou ale co jump instrukce?
    - jak vime jaka bude dalsi instrukce
    - doted jsme predpokladali sekvenci exekuci tj. `PC += 4` (ARM)
  ...
  - zajimavou alternacivou k `if (x) { y = z; }` je na x86 instrukce `y = cmov(x, z)`
    ...
    - `cmov` ale muze vytvorit nove datove zavislosti
      - `x <- cmov(x, y)`
    - neprijde mi to az  tak zajimave protoze napr. ARM je na podminene exekuci instrukci zalozen
...
- False sharing (falesne sdileni)
  ...
  - lze resit tak ze dane jedna promena = jedna cache line (nic moc reseni)
  ...
  - TSX je imeplementace HW transakcni pameti od Intelu
    ...
    - HW pouziva L1 cache pro buffering transakcnich zapisu
      ...
      - v kodu programator oznaci prvni instrukci KS (= kriticka sekce)
        - pokud se transakce nepovede, jadro skozi zpet na tuto instrukci (= Fallback Path)
      - pote co jadro detekuje zacatek KS, vsechny instrukce jsou zapisovany do lokalni pameti
    - ma dva mody
      - HLE (= HW lock ellision)
        - kompatibilat se starsim kodem
        ...
        - pokud dojde tak cely proces ukonci a zacne znovu
          - jinak receno: pri prvni vstupu do KS je takovy ze bere zamek jako odemceny
        - fallback path zacina isntrukci `cmpxchg` kvuli `xacquire` (protoze se pri prvni spusteni provede jako kdyby byla KS odemcena)
