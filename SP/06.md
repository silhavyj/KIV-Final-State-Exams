...
- semafor
      ...
      - naivni pristup
        ...
        - ale?
          ...
          - co kdyz je vybrane vlakno blokovane jeste nad necim jinym napr. klavesnice, debugger nebo sitova karta?
            ...
            - kernel musi najit jine vlakno ktere muze jit promo do fronty runnable z fronty blokovanych vlaken nad danym semaforem
  ...
  - priklad pouziti semaforu - porducent-konzument
    - buffer o omezene velikosti `buff[n]`
      - muzem zapsat n polozek naraz - sdileny zdroj
...
- mutex
  - mutex != binarni semafor protoze ma koncept vlastnictvi
    - pouze vlakno ktere mutex zamklo ho muze odemknout
...
- zpravy
  ...
  - pro UI je zpracovani povinne - funkce `wmain()`
    ...
    - GUI ma hlavni smycku kde prijima zpravy (`WindowProcedure`)
      - konzolova aplikaci to muze/nemusi implementovat
  ...
  - posilani zprav
    ...
    - PostMessage() - implementace
      ...
      - OS smaze zpravu z fronty zprav daneho vlakna tesne pred tim nez zavola `WindowProcedure()` ktery ji zpravuje
...
- signaly
  ...
  - procesne rizene signaly (Process-Directed Signals)
    ...
    - pri pouziti `pthread_sigmask()` muzeme ignorovat signaly pro sebe i vsechny sve potomky
      ...
      - mnozina vlaken mapovane na vlakno jadra (stejne PID) se redukuju pouze na jedno vlakno
  ...
  - implementace signalu
    - signaly per perocess/thread
      ...
      - fronta signalu ktere cekaji na zpracovani
  - real-time signaly
